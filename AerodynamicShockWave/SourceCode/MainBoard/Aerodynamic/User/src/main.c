...
void debug_task(void *pvParameters)
{
	...
	while(1)
    {
	   ...
	   printf("主缸气压电压值:%dmv\r\n",SystemParams.uiMasterAirCylinderAdc);

	   //y=6.48x-103  x气压值    y电压值
       if(SystemParams.uiMasterAirCylinderAdc != 0)
	   {
           master_pressure_value = (float)((float)(SystemParams.uiMasterAirCylinderAdc+103)/6.48); 
	   }
       else
       {
